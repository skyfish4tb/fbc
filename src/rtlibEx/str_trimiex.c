...
FBCALL FBSTRING *fb_TrimIEx( FBSTRING *src, FBSTRING *pattern )
{
	FBSTRING *dst;
	ssize_t len;
	char *src_ptr = NULL;
	...
	if( src->data != NULL )
    {
        ...
        if( len >= len_pattern ) 
        {
            if( len_pattern==1 ) 
            {
                src_ptr = fb_hStrSkipCharI( src_ptr,
                                     	   len,
            ...
            {                
                ...
                {
                    if( FB_MEMICMP( src_ptr, pattern->data, len_pattern )!=0 )
                        break;
                    src_ptr += len_pattern;
                    len -= len_pattern;
                }
        ...
        if( len >= len_pattern ) 
        {
            ...
            else if( len_pattern != 0 ) 
            {
                ...
                while (len >= len_pattern ) 
                {
                    if( FB_MEMICMP( src_ptr + test_index,
                                   pattern->data,
                                   len_pattern )!=0 )
                        break;

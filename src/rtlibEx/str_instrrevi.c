	...
	)
{
	...
	while ( i != 0 ) 
	{
		char ch1 = FB_TOLOWER(pachPattern[len_pattern-i]);
		while ( j <= len_pattern && ch1 != FB_TOLOWER(pachPattern[len_pattern-j]) ) 
		{
			if( bm_gc[j]==0 )
				bm_gc[j] = j - i;
			j = suffixes[j];
	...
	{
		j = len_pattern;
		while( j != 0 && FB_CHAREQUAL(pachPattern[len_pattern-j] , pachText[len_text - i - j])!=0 )
			--j;
		if( j==0 ) {
			return len_text - len_pattern - i + 1;
		} else {
			char chText = FB_TOLOWER(pachText[len_text - i - j]);

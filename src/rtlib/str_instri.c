	...
	)
{
	...
	for( i=0; i!=len_pattern; ++i )
		qs_bc[ FB_CHAR_TO_INT(FB_TOLOWER(pachPattern[i])) ] = len_pattern - i;

	/* search for string */
	for (i=start;
		i<max_size;
		i+=qs_bc[ FB_CHAR_TO_INT(FB_TOLOWER(pachText[ i + len_pattern ])) ])
	{
		if( FB_MEMICMP( pachPattern, pachText + i, len_pattern )==0 ) {
			return i + 1;
		}
	}
	...
	)
{
	...
	for( i=0; i!=len_pattern; ++i )
			bm_bc[ FB_CHAR_TO_INT(FB_TOLOWER(pachPattern[i])) ] = i;
	...
	while ( i!=0 ) 
	{
		char ch1 = FB_TOLOWER(pachPattern[i-1]);
		while ( j<=len_pattern && ch1!=FB_TOLOWER(pachPattern[j-1]) ) 
		{
			if( bm_gc[j]==0 )
				bm_gc[j] = j - i;
			j = suffixes[j];
	...
	while( i <= len_max ) 
	{
		...
		while( j!=0 && FB_CHAREQUAL(pachPattern[j-1], pachText[i+j-1])!=0 )
			--j;
		...
		{
			char chText = FB_TOLOWER(pachText[i+j-1]);
	...
	if( start < imax )
	{
		for( i=start; i != imax; ++i ) {
			...
			for( j=0; j!=len_pattern; ++j ) {
				if( FB_CHAREQUAL(pachText[j]), pachPattern[j])==0 )
					break;
			}
			if( j==len_pattern )
				return i + 1;
...
FBCALL ssize_t fb_StrInstrI( ssize_t start, FBSTRING *src, FBSTRING *patt )
{
	...
	else
	{
		...
		{
			const char *pszEnd = (const char *) FB_MEMICHR( src->data + start - 1, patt->data[0], size_src - start + 1);
			if( pszEnd==NULL )
			{
				r = 0;
			}

...
SUITE( fbc_tests.wstring_.rtrimIW_ )

	#macro check( a, length, expected )
		scope
			dim wr as wstring * 50 = rtrimI( wstr(a) )
		...
		scope
			...
			dim we as wstring * 50 = wstr( expected )
			dim wr as wstring * 50 = rtrimI( wa )
	...
	#macro check_filter( a, filter, length, expected )
		scope
			dim wr as wstring * 50 = rtrimI( wstr(a), wstr(filter) )
			dim we as wstring * 50 = wstr( expected )
		...
		scope
			...
			dim wr as wstring * 50 = rtrimI( wa, wf )
			dim we as wstring * 50 = wstr( expected )
	...
	#macro check_filter_any( a, filter, length, expected )
		scope
			dim wr as wstring * 50 = rtrimI( wstr(a), any wstr(filter) )
			dim we as wstring * 50 = wstr( expected )
		...
		scope
			dim wa as wstring * 50 = wstr(a)
			dim wf as wstring * 50 = wstr(filter)
			dim wr as wstring * 50 = rtrimI( wa, any wf )
			dim we as wstring * 50 = wstr( expected )

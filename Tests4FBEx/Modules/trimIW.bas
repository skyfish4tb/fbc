...
SUITE( fbc_tests.wstring_.trimIW_ )

	#macro check( text, length, expected )
		scope
			dim wr as wstring * 50 = trimI( wstr(text) )
	...
	#macro check_filter( text, pattern, length, expected )
		scope
			dim wr as wstring * 50 = trimI( wstr(text), wstr(pattern) )
			dim we as wstring * 50 = wstr( expected )
		...
		scope
			...
			dim wr as wstring * 50 = trimI( wt, wf )
			dim we as wstring * 50 = wstr( expected )
	...
	#macro check_filter_any( text, pattern, length, expected )
		scope
			dim wr as wstring * 50 = trimI( wstr(text), any wstr(pattern) )
			dim we as wstring * 50 = wstr( expected )
		...
		scope
			dim wt as wstring * 50 = wstr( text )
			dim wf as wstring * 50 = wstr( pattern )
			dim wr as wstring * 50 = trimI( wt, any wf )
			dim we as wstring * 50 = wstr( expected )

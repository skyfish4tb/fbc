#include "fbcunit.bi"
#include once "inc\chk-wstring.bi"

SUITE( fbc_tests.wstring_.ltrimIW_ )

	#macro check( text, length, expected )
		scope
			dim wr as wstring * 50 = ltrimI( wstr(text) )
		...
		scope
			...
			dim we as wstring * 50 = wstr( expected )
			dim wr as wstring * 50 = ltrimI( wt )
	...
	#macro check_filter( text, pattern, length, expected )
		scope
			dim wr as wstring * 50 = ltrimI( wstr(text), wstr(pattern) )
			dim we as wstring * 50 = wstr( expected )
		...
		scope
			...
			dim wr as wstring * 50 = ltrimI( wt, wf )
			dim we as wstring * 50 = wstr( expected )
	...
	#macro check_filter_any( text, pattern, length, expected )
		scope
			dim wr as wstring * 50 = ltrimI( wstr(text), any wstr(pattern) )
			dim we as wstring * 50 = wstr( expected )
		...
		scope
			dim wt as wstring * 50 = wstr( text )
			dim wf as wstring * 50 = wstr( pattern )
			dim wr as wstring * 50 = ltrimI( wt, any wf )
			dim we as wstring * 50 = wstr( expected )
